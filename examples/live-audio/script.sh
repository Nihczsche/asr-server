...
export NEMO__MODEL=nvidia/parakeet-rnnt-1.1b # or Systran/faster-whisper-tiny.en if you are running on a CPU for a faster inference.

# Ensure you have `asr-server` running. If this is your first time running it expect to wait up-to a minute for the model to be downloaded and loaded into memory. You can run `curl localhost:8000/health` to check if the server is ready or watch the logs with `docker logs -f <container_id>`.
docker run --detach --gpus=all --publish 8000:8000 --volume ~/.cache/huggingface:/root/.cache/huggingface --env NEMO__MODEL=$NEMO__MODEL fedirz/asr-server:latest-cuda
# or you can run it on a CPU
# docker run --detach --publish 8000:8000 --volume ~/.cache/huggingface:/root/.cache/huggingface --env NEMO__MODEL=$NEMO__MODEL fedirz/asr-server:latest-cpu
